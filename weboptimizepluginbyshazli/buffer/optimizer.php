	...
	class ALOptimizer {
			...
			if ($matched) {
				foreach ($matches[0] as $link_tag) {
					...
					foreach ($css_config as $config) {
						...
						if (!empty($stripped_search_for) && !empty($stripped_link_tag)) {
						...
						} else if (!empty($search_for) && strpos($link_tag, $search_for) !== false) {
							$replace_attributes = $this->getLinkAttributes($replace_with);
							$new_href = isset($replace_attributes['href']) ? $replace_attributes['href'] : $original_href;
							break;
						}
						if (!empty($search_for_regex) && preg_match($search_for_regex, $link_tag)) {
							$replace_attributes = $this->getLinkAttributes($replace_with);
							$new_href = isset($replace_attributes['href']) ? $replace_attributes['href'] : $original_href;
							break;
						}
		...
		public function addCriticalCssV2($buffer, $critical_css_v2) {
			if (ALHelper::safePregMatch('/<head\b[^>]*>/iU', $buffer, $matches)) {
				...
				$buffer = ALHelper::safePregReplace('/' . preg_quote($head_tag, '/') . '/', $replacement, $buffer, 1);
			}
			return $buffer;
		}
